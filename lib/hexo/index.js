...
var tildify = require('tildify'); //转换目录路径为相对用户主目录路径
var Database = require('warehouse'); //json数据库
var _ = require('lodash'); //一个强大的工具包,类似python的numpy,提供了数组集合字符串对象等实用方法
var chalk = require('chalk'); //处理字符串颜色和格式的工具包
var EventEmitter = require('events').EventEmitter;
var fs = require('hexo-fs');
var Module = require('module'); //Nodejs核心module包
var vm = require('vm'); //Nodejs核心包,处理与V8引擎相关上下文
...
Hexo.prototype.init = function () {
  ...
  return Promise.each([
    ...
    return self.execFilter('after_init', null, {
      context: self
    });
  }).then(function () {
    // Ready to go!
...
Hexo.prototype._generate = function (options) {
  ...
  return this.execFilter('before_generate', null, {
      context: this
    })
    .then(function () {
    ...
    .map(function (item) {
      ...
      return self.execFilter('template_locals', locals, {
          context: self
        })
      ...
      return self.execFilter('after_generate', null, {
        context: self
      });
    }).finally(function () {
...
Hexo.prototype.exit = function (err) {
  ...
  return this.execFilter('before_exit', null, {
    context: this
  }).then(function () {
    self.emit('exit', err);
  });
